  ...
  return (
    <Shell>
      <div className="page-header flex-between">
        <div>
          <h1 className="page-title">DRE — Demonstração do Resultado</h1>
      ...
      {loading ? (
        ...
        <div className="card" style={{ textAlign: 'center', padding: 60 }}>
          ...
          <div style={{ fontFamily: 'var(--font-sub)', fontWeight: 600, fontSize: 15 }}>
            Sem dados classificados para {MONTH_NAMES[month]}/{year}
          </div>
          ...
          <div className="metrics-grid mb-6">
            {[
              ...
              <div className="metric-card" key={m.label}>
                ...
                <div className={`metric-value ${m.value < 0 ? 'negative' : ''}`} style={{ fontSize: 18 }}>
                  {fmt(m.value)}
                </div>
                {m.pctVal && (
                  <div style={{ fontSize: 12, color: 'var(--brave-gray)', marginTop: 2 }}>
                    {m.pctVal} da receita bruta
                  </div>
                )}
              </div>
            ))}
          ...
          <div className="grid-2 mb-6">
            ...
            <div className="card">
              <div style={{ fontFamily: 'var(--font-sub)', fontWeight: 600, fontSize: 13, marginBottom: 4 }}>
                DRE Detalhada — {MONTH_NAMES[month]}/{year}
              </div>
              <div style={{ fontSize: 11, color: 'var(--brave-gray)', marginBottom: 20 }}>
                % calculado sobre Receita Bruta
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                {dre.lines.map((line: any, i: number) => (
                  ...
                  >
                    <div>
                      <div style={{
                        fontSize: line.highlight ? 13 : 12,
                        fontWeight: line.highlight ? 700 : 400,
                        fontFamily: line.highlight ? 'var(--font-sub)' : 'var(--font-body)'
                      }}>{line.label}</div>
                      {line.sublabel && (
                    ...
                    <div style={{ textAlign: 'right' }}>
                      <div style={{
                        ...
                        color: line.value >= 0 ? (line.highlight ? 'var(--brave-dark)' : '#1a7a4a') : '#c0392b'
                      }}>
                        {fmt(line.value)}
                      </div>
            ...
            <div className="card">
              ...
              <div style={{ display: 'flex', gap: 16, marginTop: 8, justifyContent: 'center' }}>
                {[
                  { color: '#2b2d42', label: 'Rec. Bruta' },
                  { color: '#8d99ae', label: 'Res. Bruto' },
                  { color: '#eaca2d', label: 'Res. Líquido' },
                ].map(l => (
                ...
                <tbody>
                  {(data?.yearData || []).map((d: any, i: number) => (
                    <tr key={i} style={{ background: i + 1 === month ? 'rgba(234,202,45,0.08)' : '' }}>
                      ...
                      <td style={{ textAlign: 'right', fontSize: 12, color: d.resultadoBruto < 0 ? '#c0392b' : '' }}>
                        {d.receitaBruta > 0 ? fmt(d.resultadoBruto) : '—'}
                      </td>
                      <td style={{ textAlign: 'right', fontSize: 12, color: d.resultadoOperacional < 0 ? '#c0392b' : '' }}>
                        {d.receitaBruta > 0 ? fmt(d.resultadoOperacional) : '—'}
                      </td>
                      <td style={{ textAlign: 'right', fontSize: 12, fontWeight: 600, color: d.resultadoLiquido < 0 ? '#c0392b' : '#1a7a4a' }}>
                        {d.receitaBruta > 0 ? fmt(d.resultadoLiquido) : '—'}
                      </td>
                      <td style={{ textAlign: 'right', fontSize: 12, color: 'var(--brave-gray)' }}>
                        {d.receitaBruta > 0 ? pct(d.resultadoLiquido, d.receitaBruta) : '—'}
                      </td>
                    </tr>
                  ))}
                </tbody>
